...
//                let phone:CNLabeledValue = cnContact.phoneNumbers.first?.value
                let PhNo:CNLabeledValue = cnContact.phoneNumbers.first!;
                let MobNumVar  = (PhNo.value ).value(forKey: "digits") as? String
                self.phone = MobNumVar
            }
        }
    }
}
