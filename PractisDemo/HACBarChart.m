...
-(UILabel *) setLabelWithFrame:(CGRect)frame text:(NSString*)text barColor:(UIColor *)barColor progress:(CGFloat)progess{
    ...
    NSLog(@"text is %f",progess);
    //UserDefaults.standard.setValue("no", forKey: "isNagative")
    NSString *value = [[NSUserDefaults standardUserDefaults] valueForKey:@"isNagative"];
    
    if (progess < 0.00)
    {
        if ([value  isEqual: @"yes"])
        {
            NSString *newvalue = [NSString stringWithFormat:@"-%@",text];
            progressText.text = newvalue;
        }
        return progressText;
    }else if (progess == 0.00){
        progressText.text = @"";
        return progressText;
    }
